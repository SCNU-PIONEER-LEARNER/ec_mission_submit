 ...
 int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  uint8_t* data_copy = pvPortMalloc(*Len);
  uint32_t len_copy = *Len;
  
  if(data_copy) {
    memcpy(data_copy, Buf, *Len);
    xQueueSend(messageQueue1, &data_copy, 0);
    xQueueSend(messageQueue2, &len_copy, 0);
  }
  return (USBD_OK);
  /* USER CODE END 6 */
}
...
int usbReceive(uint8_t* buf_, uint16_t len_) {
  uint16_t result = 0;
